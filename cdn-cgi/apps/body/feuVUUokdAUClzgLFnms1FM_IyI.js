;if(CloudflareApps.matchPage(CloudflareApps.installs['ZkwtTeqlU4fE'].URLPatterns)){(function(modules){var installedModules={};function __webpack_require__(moduleId){if(installedModules[moduleId]){return installedModules[moduleId].exports;}
...
var hash=void 0
var options=CloudflareApps.installs['ZkwtTeqlU4fE'].options
var product=CloudflareApps.installs['ZkwtTeqlU4fE'].product
...
if("ZkwtTeqlU4fE"==='preview'&&node.nodeName==='IFRAME'&&node.src.indexOf('https://embedded.cloudflareapps.com')!==-1){return}
...
if("ZkwtTeqlU4fE"==='preview'){messageIndex=previewMessageIndex}else{messageIndex=Math.floor(Math.random()*options.messages.length)}
...
if("ZkwtTeqlU4fE"!=='preview'&&(!shouldShow||hasSeenHash())){hideWelcomeBar()
return}
updateElementStyle()
element.innerHTML=''
...
window.CloudflareApps.installs['ZkwtTeqlU4fE'].scope={updateOptions:function updateOptions(nextOptions){if(nextOptions.messages.length!==options.messages.length){previewMessageIndex=nextOptions.messages.length-1}else{for(var i=0;i<nextOptions.messages.length;i++){var oldEntry=options.messages[i]
